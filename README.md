...
<<<<<<< HEAD
### `[[In Version v1.1.1]]` 支持来回滚动的操纵历史数据

​		在版本v1.1.0中，支持在正式标注软件中进行左右回滚调整数据的模式，对于标注结束的图片，如果您后悔了您的标注，可以摁`<-`键或者是A键（值得注意的是，本次更新修正了正式标注界面中快捷键A映射前进的反人类快捷键，改向了正常的A键回退，D键前进的快捷键方式。当然，可能更习惯用的是`<-` 和`->`键）
=======
