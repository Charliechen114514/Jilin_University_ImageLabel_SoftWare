...
QString curPicForLabeling_MainWindow::writingMethod()
{
    ...
    switch(saveWay)
    {
        ...
        case saveMethod::YOLO:
        {
            for(int i = 0; i < finalSigCurPicInfo.first.size();i++)
            {
                resultToText += QString::number(finalSigCurPicInfo.second[i].first);
                for(int j = 0; j < finalSigCurPicInfo.first[i].size(); j++)
                {
                  resultToText += " " + QString::number(getWIDRATIO(finalSigCurPicInfo.first[i][j].x())) + \
                                  " " + QString::number(getHEIGHTRATIO(finalSigCurPicInfo.first[i][j].y()));
                }
                resultToText += "\n";
            }
        }break;
