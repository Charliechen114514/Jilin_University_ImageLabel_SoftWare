...
void MainWindow::preLoad()
{
    ...
    else
    {
        ...
        this->statusLabelInfo->setText(QString("找不到conf文件，请将他放置在:")+DEF_CONFFILE_PATH);
        return;
    }
    // debug node
    ...
    this->statusLabelInfo->setText(QString("找到conf文件!"));
}

void MainWindow::preLoadStatusBar()
{
    statusLabelInfo = new QLabel;
    ui->statusbar->addWidget(statusLabelInfo);
}
...
void MainWindow::initDefaultPictures()
{
    ...
    if(!defLoadImagePath.isEmpty())
    {
        ...
        // qDebug() << getDirPath;
        if(!getDir.exists()){
            QMessageBox::critical(this, QString("打开文件夹失败！:("),QString("没有办法打开文件夹:)" + defLoadImagePath + "查看一下文件夹姓名是不是有问题"));
        ...
        statusTxt += QString("成功导入默认路径下的图片");
        if(!isIgnoreFailed)
        {
    ...
    {
        statusTxt += QString("无法导入默认文件夹：") +defLoadImagePath + "可能时文件路径错误！";
...
LABLE_LOADS:
    ...
    if(!defLoadLabelPath.isEmpty())
    {
        ...
        statusTxt += "加载默认路径下的标签文件成功！";
        updateCurrentLabelCheckText();
    }
    else
    {
        if(defLoadLabelPath.isEmpty()){
            statusTxt += QString("标签默认加载路径为空！请检查: ") + DEF_CONFFILE_PATH;
        }
        else
        {
            statusTxt += "但是未检测到路径:" + defLoadLabelPath + "有标签文件";
        }
    }

    this->statusLabelInfo->setText(statusTxt);
}
...
void MainWindow::getDirectoryFromUsr()
{
    ...
    curViewPic.load(FilePathAll.first());
    pathPics.append(FilePathAll);
    ui->curPlaceProcessBar->setRange(0,pathPics.size());
    curViewPicIndex = pathPics.size();
    imgShow(pathPics.size()-1);
