...
// status bar Message
#define STATUSBAR_MSG(X) QString(X)
#define DEF_STATUS_COLOR "red"
...
// 默认的保存方式
#define DEF_SAVE_METHOD saveMethod::TRIDITIONAL

// 显示信息的Revalent Micro
#define TAB QString("   ")
#define SHOWNUM(X) TAB + QString("点") + QString::number(X) + QString(":")
#define SHOWPOINT(X,Y) QString("(") + QString::number(X) +QString(", ") +  QString::number(Y) + QString(")")
...
public:
    ...
    QList<CurPolyPoints>                notLastPicPoly;
    /*专门存放种类标签*/
    ...
    CurPolyPoints                       tempPointsList;
    /* 最新单张图片的所有信息 */
    curPoints_LabelPair_Pair            finalSigCurPicInfo;
    /* 之前的土拍你修改的所有信息 */
    curPoints_LabelPair_Pair            notFinalSigCurPicInfo;
    /* 图片链表信息 */
    QList<curPoints_LabelPair_Pair>     curPicListInfo;
    // 设置保存TXT格式问题
    void                                setTXTFormYOLO(){saveWay = saveMethod::YOLO;}
    void                                setTXTFormTriditional(){saveWay = saveMethod::TRIDITIONAL;}
    /* showInfoInWindow:显示当前窗口下所有的标点对应标签的窗口 */
    void                                showCurPicAllInfo();
    // debug
    void                                displayCurPicInfo(int index);
    // 设置状态栏颜色
    void                                statusBarInit();
    /* 更改显示信息状态 */
    void                                switchShowOrHide();
    void                                switch_ShowOrHideMenuInfo();
protected:
    /*重写窗体重绘事件*/
...
private:
    ...
    /* 友好的提示模式：认真 */
    bool                                isOpenCheckInWrite;
    /* 保存格式 */
    saveMethod                          saveWay;
    /* 开不开保存的Info */
    bool                                shallShowSaveRevelantInfo;
private slots:
    /* 将标签重新加回到 */
