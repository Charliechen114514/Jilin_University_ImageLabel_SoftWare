...
void curPicForLabeling_MainWindow::displayCurPicInfo(int index)
{
    QString curPolyInfo;

        curPoints_LabelPair_Pair curPicInfo = curPicListInfo[index];
        for(int i = 0; i < curPicInfo.first.size();i++)
        {
            curPolyInfo += "标签代号：" + QString::number(curPicInfo.second[i].first) + "  "\
                        "标签名称：" + curPicInfo.second[i].second + "\n";
            for(int j = 0; j < curPicInfo.first[i].size();j++)
            {
                curPolyInfo += SHOWNUM(j) + SHOWPOINT(curPicInfo.first[i][j].x(), curPicInfo.first[i][j].y()) + "\n";
            }
        }
        QMessageBox::information(this,"当前窗口信息",curPolyInfo);
}

#define getWIDRATIO(number) ((double)number/this->width())
#define getHEIGHTRATIO(number) ((double)number/this->height())
/**************************************************************************************************
*
    ...
    ui(new Ui::curPicForLabeling_MainWindow)
{
    ...
    ticks = 0;
    curViewIndex = 0;
    saveWay = DEF_SAVE_METHOD;
    shallShowSaveRevelantInfo = true;
    switch_ShowOrHideMenuInfo();
    initBasicConnection();
    /*显示这个窗口*/
    setCurMaxPointCountWindows->show();
}

void curPicForLabeling_MainWindow::statusBarInit()
{
    statusInfoLabel = new QLabel;
    statusInfoLabel->setStyleSheet(QString("color: ") + DEF_STATUS_COLOR);
    ui->statusbar->addWidget(statusInfoLabel);
}

void curPicForLabeling_MainWindow::switchShowOrHide()
{
    shallShowSaveRevelantInfo = !shallShowSaveRevelantInfo;
    switch_ShowOrHideMenuInfo();
}

void curPicForLabeling_MainWindow::switch_ShowOrHideMenuInfo()
{
    if(shallShowSaveRevelantInfo)
    {
        ui->action_showOrHideSaveInfo->setText("关闭保存时弹出的写入信息提示");
    }
    else
    {
        ui->action_showOrHideSaveInfo->setText("打开保存时弹出的写入信息提示");
    }
}
...
void curPicForLabeling_MainWindow::initBasicConnection()
{
    ...
    /* 设置保存方式 */
    connect(ui->action_YOLO,&QAction::triggered,this,&curPicForLabeling_MainWindow::setTXTFormYOLO);
    connect(ui->action_traditionalDebug,&QAction::triggered,this,&curPicForLabeling_MainWindow::setTXTFormTriditional);
    /* 链接是否显示 Debug 信息*/
    connect(ui->action_showOrHideSaveInfo, &QAction::triggered,this,&curPicForLabeling_MainWindow::switchShowOrHide);
    /* 链接：查看信息 */
    connect(ui->action_checkCurPicInfo,&QAction::triggered,this,&curPicForLabeling_MainWindow::showCurPicAllInfo);
}
...
{
    qDebug() << "从" << curViewIndex;
    curViewIndex++;
    qDebug() << "移动到" << curViewIndex;
    qDebug() << curPicListInfo.size();
    if(curViewIndex < curPicListInfo.size())
    {
        curPicListInfo.replace(curViewIndex-1, notFinalSigCurPicInfo);
#if DEBUG
        displayCurPicInfo(curViewIndex-1);
#endif
        if(curViewIndex != curPicListInfo.size())
        {
            notFinalSigCurPicInfo = curPicListInfo[curViewIndex];
        }
        notLastPicPoly.clear();
        LABEL_SAVE_DEF_METHOD;
        goto NOT_LABEL_LATEST;
    }
    if(curViewIndex >= WholeGroupPicsPathList.size()){
        curPicListInfo.push_back(finalSigCurPicInfo);
    ...
    if(isSave == CCSTDC_JLU_IMAGE_LABLE_UNSAVE)
    {
        ...
        if(QMessageBox::Yes == QMessageBox::question(this,"嗯？哪里跑？","小子跑路这快，不保存一下?不然会全部消失！点击确认以切到保存全部"))
        {
            curPicListInfo.push_back(finalSigCurPicInfo);
            LABEL_SAVE_DEF_METHOD;
            emit finishEveryThingAndReturnsTheNewLyLabelPair();
        ...
        {
            // USR FRIENDLY
            // if there is a null finalSigCurPicInfo,don;t move
            if(!finalSigCurPicInfo.first.empty())
            {
                curPicListInfo.push_back(finalSigCurPicInfo);
            }
            else
...
FINISH_JUDGE:
    ...
    notFinalSigCurPicInfo.first.clear();
    notFinalSigCurPicInfo.second.clear();
    tempPointsList.clear();
    curPicPoly.clear();
NOT_LABEL_LATEST: // flags are used especially when you move to previous pics and move backward then
    if(WholeGroupPicsPathList.size() > curViewIndex)
    {
...
void curPicForLabeling_MainWindow::moveToPreviousPic()
{
    // 如果是最后一张，选择pushBack方法
    if(curViewIndex == curPicListInfo.size())
    {
        curPicListInfo.push_back(finalSigCurPicInfo);
    }
    ...
    curViewIndex--;
    qDebug() << "移动到" << curViewIndex;
    // 将当前图片数据处理到 notFinalSigCurPicInfo 上去
    notFinalSigCurPicInfo = curPicListInfo[curViewIndex];
    if(curViewIndex < 0){
        QMessageBox::information(this,"我也不知道我应该给这个窗口起一个什么标题","哥们已经到了第一张了，别摁上一张了！");
        curViewIndex = 0;
        return;
    }
...
void curPicForLabeling_MainWindow::paintEvent(QPaintEvent*)
{
    ...
    {
        // New added: for not the HEAD Picture Label
        if(curViewIndex < curPicListInfo.size())
        {
            for(int i = 0; i < notFinalSigCurPicInfo.first.size();i++)
            {
                QRect thisRect;
                thisRect.setTopLeft(notFinalSigCurPicInfo.first[i][0]);
                thisRect.setBottomRight(notFinalSigCurPicInfo.first[i][1]);
        ...
        {
            // REMINDER: OLD CODE PIECE TO Show curPic Points
    ...
    default:
    {
        ...
        if(curViewIndex < curPicListInfo.size())
        {
            for(int i = 0; i < notFinalSigCurPicInfo.first.size();i++)
            {
                painter.drawText(notFinalSigCurPicInfo.first[i][0] + QPoint(LABEL_INDEX_TEXT_SHOW_OFFSET_X,LABEL_INDEX_TEXT_SHOW_OFFSET_Y),QString::number(i + 1));
                for(int j = 0; j < notFinalSigCurPicInfo.first[i].size()-1;j++)
                {
                    painter.drawLine(notFinalSigCurPicInfo.first[i][j],notFinalSigCurPicInfo.first[i][j + 1]);
                }
                painter.drawLine(notFinalSigCurPicInfo.first[i][notFinalSigCurPicInfo.first[i].size()-1],notFinalSigCurPicInfo.first[i][0]);
                update();
            }
    ...
    if(e->button() == Qt::LeftButton){
        ...
        case 5:
        {
            if(ticks < curAllowMaxPointsCount - 1)
            {
                ...
                statusInfoLabel->setText(STATUSBAR_MSG("当前你已经标注了: " + QString::number(ticks) + "个点,共计"+ QString::number(curAllowMaxPointsCount)+ "个点,共计" ));
            }
            else
            {
                //qDebug() << "第"<< ticks <<"位置点已找好";
                //qDebug() << e->pos();
                statusInfoLabel->setText(STATUSBAR_MSG("该轮廓标记结束!"));
                ticks = 0;
                //curPicPoly.last().push_back(e->pos());
                tempPointsList.push_back(e->pos());
                if(curViewIndex < curPicListInfo.size())
                {
                    notLastPicPoly.push_back(tempPointsList);
                }
                else
                {
...
{
    // AFTER ALL
    // 貌似切换成switch是acceptable的，但是懒得改史山
...
void curPicForLabeling_MainWindow::pushBackToFinalSigCurPicInfo()
{
    if(curViewIndex < curPicListInfo.size())
    {
        notFinalSigCurPicInfo.first.push_back(notLastPicPoly.back());
        notFinalSigCurPicInfo.second.push_back(dialog->returnFinalLabelToOutward());
    }
    else
    {
        // to the new pics, we use push_back
...
void curPicForLabeling_MainWindow::saveForResultCurPicButLabelOnly()
{
    ...
    if(shallShowSaveRevelantInfo)
        QMessageBox::information(this,"debug",QString("第") + QString::number(curViewIndex) + "\n" + resultToText);
    ...
    if(isAlwaysSaveConstPlaceForLabels)
    {
        ...
        if(shallShowSaveRevelantInfo)
            QMessageBox::information(this,"debug",SavePath);
...
{
    if(curViewIndex < curPicListInfo.size() - 1)
    {
        if(!notFinalSigCurPicInfo.first.empty())
        {
            notFinalSigCurPicInfo.first.pop_back();
            notFinalSigCurPicInfo.second.pop_back();
            qDebug() << "对于非前卫图片的撤销";
        }
    }
    else
...
QString curPicForLabeling_MainWindow::writingMethod()
{
    ...
    switch(saveWay)
    {
        case saveMethod::TRIDITIONAL:
        {
            for(int i = 0; i < curPicListInfo[curViewIndex-1].first.size();i++)
            {
                resultToText += LABLE_TEXT_INDEX_PREFIX + QString::number(curPicListInfo[curViewIndex-1].second[i].first) + \
                                LABLE_TEXT_INSERT_SLASH + LABLE_TEXT_LABLENAME_PREFIX + curPicListInfo[curViewIndex-1].second[i].second + LABLE_TEXT_INSERT_SLASH;
                QString pointsSetShow = LABLE_TEXT_POINTSET_SHOW_PREFIX + LABLE_TEXT_INSERT_SLASH ;
                for(int j = 0 ; j < curPicListInfo[curViewIndex-1].first[i].size(); j++)
                {
                  pointsSetShow += LABLE_TEXT_POINT_SHOW_PREFIX + QString::number(curPicListInfo[curViewIndex-1].first[i][j].x())\
                                 + LABLE_TEXT_POINT_SPLIT + QString::number(curPicListInfo[curViewIndex-1].first[i][j].y()) +LABLE_TEXT_POINT_SHOW_SUFFIX+ LABLE_TEXT_INSERT_SLASH;
                }
        ...
        case saveMethod::YOLO:
        {
            for(int i = 0; i < curPicListInfo[curViewIndex-1].first.size();i++)
            {
                resultToText += QString::number(curPicListInfo[curViewIndex-1].second[i].first);
                for(int j = 0; j < curPicListInfo[curViewIndex-1].first[i].size(); j++)
                {
                    resultToText += " " + QString::number(getWIDRATIO(curPicListInfo[curViewIndex-1].first[i][j].x())) + \
                                    " " + QString::number(getHEIGHTRATIO(curPicListInfo[curViewIndex-1].first[i][j].y()));
                }
                resultToText += "\n";
            }
        }break;
        default:
            QMessageBox::critical(this,"WHAT THE F**K?","你是怎么到达这里的？？？");
            break;
    }
    return resultToText;
}
...
// showInfo
void curPicForLabeling_MainWindow::showCurPicAllInfo()
{
    // get current pic info
    // doubt: IO读取还是程序存储
    if(curViewIndex > curPicListInfo.size()){
        QMessageBox::critical(this,"Error","啥都没有火速爬");
        return;
    }
    ...
    if(curViewIndex == curPicListInfo.size())
    {
        ...
        for(int i = 0; i < finalSigCurPicInfo.first.size();i++)
        {
            curPolyInfo += "标签代号：" + QString::number(finalSigCurPicInfo.second[i].first) + "  "\
                       "标签名称：" + finalSigCurPicInfo.second[i].second + "\n";
            for(int j = 0; j < finalSigCurPicInfo.first[i].size();j++)
            {
                curPolyInfo += SHOWNUM(j) +
                               SHOWPOINT(finalSigCurPicInfo.first[i][j].x(),
                                         finalSigCurPicInfo.first[i][j].y()) + "\n";
            }
        }
        for(int i = 0; i < tempPointsList.size();i++)
        {
            curPolyInfo += "正在标注的轮廓：";
            curPolyInfo += SHOWNUM(finalSigCurPicInfo.first[i].size()) +
                           SHOWPOINT(tempPointsList[i].x(), tempPointsList[i].y());
    ...
    else // 针对不是那些最前沿的
    {
        for(int i = 0; i < notFinalSigCurPicInfo.first.size();i++)
        {
            curPolyInfo += "标签代号：" + QString::number(notFinalSigCurPicInfo.second[i].first) + "  "\
                        "标签名称：" + notFinalSigCurPicInfo.second[i].second + "\n";
            for(int j = 0; j < notFinalSigCurPicInfo.first[i].size();j++)
            {
                curPolyInfo += SHOWNUM(j) + SHOWPOINT(notFinalSigCurPicInfo.first[i][j].x(), notFinalSigCurPicInfo.first[i][j].y()) + "\n";
            }
        }
    ...
    // debug： determine of a better selection
    QMessageBox::information(this,"当前窗口信息",curPolyInfo);
